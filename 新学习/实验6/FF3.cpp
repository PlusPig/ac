...
int dfs(int u, int t, int flow, vector<vector<int>> &res, vector<vector<int>> &connected, vector<bool> &visited)
{
    if (u == t)
        return flow;
    visited[u] = true;
    for (int v : connected[u])
    {
        ...
        {
            int d = dfs(v, t, min(flow, res[u][v]), res, connected, visited);
            if (d > 0)
            {
                res[u][v] -= d;
                res[v][u] += d;
...
int maxflow(int s, int t, int V, vector<vector<int>> &res, vector<vector<int>> &connected)
{
    ...
    while (true)
    {
        ...
        int f = dfs(s, t, INF, res, connected, visited);
        if (f == 0)
            break;
        flow += f;
    }
